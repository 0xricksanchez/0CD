...
def load_data(arch):
    ...
    data_db_path = os.path.join(
        current_file_path, "..", "data/stackguards", arch + ".json"
    )
...
def check_arch(platform):
    if platform not in supported:
        bn.log_error(
            "[-] This plugin only supports the following platforms: " + str(supported)
        )
        return False
...
def run_plugin_all(bv):
    if check_arch(bv.platform.name):
        syms = list(
            filter(lambda sym: "__stack_chk_fail" in sym.name, bv.get_symbols())
        )
        if len(syms) == 0:
            return 0
        ...
        for bb in function.medium_level_il:
            for insn in bb:
                ...
                for var in insn.vars_read:
                    if var.name == self.data["src"] and isinstance(
                        var.type, bn.types.PointerType
                    ):
                        vartype = bn.Type.pointer(
                            self.bv.arch,
                            bn.Type.named_type_from_registered_type(
                                self.bv, "tcbhead_t"
                            ),
                        )
                        function.create_user_var(
                            var,
                            vartype,
                            bn.Settings().get_string("0cd.stackguards.tcb_name"),
                        )
        ...
        for bb in function.medium_level_il:
            for insn in bb:
                ...
                for var in insn.vars_written:
                    if "stack_guard" in str(insn) and "tcbhead_t" in str(
                        insn.vars_read
                    ):
